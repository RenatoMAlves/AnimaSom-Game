var playState = {
    ...
    create: function(){
        ...
        function initial(){
            game.add.tween(groupCidade.children[0].scale).to( { x: 1, y: 1 }, 200, Phaser.Easing.Elastic.In, true);
            game.add.tween(cachorro.scale).to( { x: 0.5, y: 0.5 },200500, Phaser.Easing.Elastic.In, true);
            groupCidade.children[0].x -= 230; 
            cachorro.x -= 230;
            groupCidade.children[0].y += 70; 
            cachorro.y += 70;
