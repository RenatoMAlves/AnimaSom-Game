var playState = {
    ...
    create: function(){
        ...
        function initial(){
            ...
            game.add.tween(groupCidade.children[1]).to( { alpha: 1 }, 2000, Phaser.Easing.Linear.None, true);
            game.add.tween(groupCidade.children[2]).to( { alpha: 1 }, 2000, Phaser.Easing.Linear.None, true);
            game.add.tween(gato).to( { alpha: 1 }, 2000, Phaser.Easing.Linear.None, true);
            game.add.tween(passaro).to( { alpha: 1 }, 2000, Phaser.Easing.Linear.None, true);

            game.time.events.add(Phaser.Timer.SECOND * 2, groupCidade.children[1].visible = true, this);
            game.time.events.add(Phaser.Timer.SECOND * 2, groupCidade.children[2].visible  = true, this);
            game.time.events.add(Phaser.Timer.SECOND * 2, gato.visible = true, this);
            game.time.events.add(Phaser.Timer.SECOND * 2, passaro.visible = true, this);
